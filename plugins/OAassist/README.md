...
| `TLS_CERT_FILE` | _(unset)_ | PEM certificate the engine serves HTTPS with. Set together with `TLS_KEY_FILE`; unset = plain HTTP, exactly as before. Closes the cleartext LAN hop between the edge and the engine. |
| `TLS_KEY_FILE` | _(unset)_ | PEM private key for `TLS_CERT_FILE`. Required whenever the certificate is set. |
| `TLS_CLIENT_CA` | _(unset)_ | PEM CA bundle; when set, every peer (the edge, or a direct caller) must present a certificate chained to it or the TLS handshake fails. Requires `TLS_CERT_FILE`. Note the two layers: the TLS layer verifies the **peer's** chain, while the per-token thumbprint check (`issue-token --cert-thumbprint`) reads the **app's** certificate the edge forwards in `X-ARR-ClientCert` on the activity routes. |
...
OAassist validates the config at startup and refuses to start with a clear error message when: `LLM_PROVIDER=anthropic` but no key is set; `LLM_PROVIDER` or `DEPLOYMENT_MODE` is an unknown value; `DEPLOYMENT_MODE=on_prem` is combined with a non-local provider; `TLS_CERT_FILE` and `TLS_KEY_FILE` are not set together, `TLS_CLIENT_CA` is set without them, or any of the three names a file that does not exist; or any of `RELEVANCE_MAX_DISTANCE`, the `HYBRID_*` top-k values, the `SECTION_EXPANSION_MAX_*` caps, the `RATE_LIMIT_*` limits or the `LLM_WIKI_*_WEIGHT` weights is zero or negative (`HISTORY_RETENTION_DAYS` and `ACTIVITY_MAX_EVENTS_PER_USER` accept `0`, which means "keep everything").
...
**What the Azure path actually costs, measured against a live gpt-5.4 deployment (2026-08-06).** The model is not the slow part: one call completed in 779 ms, first token at 160 ms. A full `/v1/ask` on DEMO takes 8–10 s, and the difference is OAassist's own retrieval — chiefly the cross-encoder when `RERANK_ENABLED=true` with `BAAI/bge-reranker-base`, whose ~1.1 GB model also loads lazily on the *first* query after a restart (`model_loaded` in `/healthz` refers only to the embedding model). Look there before suspecting the provider. On spend: a small ask cost 89 tokens with `reasoning_tokens: 0` — a reasoning model does not necessarily deliberate on direct documentation questions — while a real ask is dominated by the expanded context (`SECTION_EXPANSION_MAX_TOTAL_CHARS`). Azure's own per-deployment quota is generous (5,000 requests and 500,000 tokens per minute on a standard deployment) and protects Microsoft's infrastructure, **not your bill**.
